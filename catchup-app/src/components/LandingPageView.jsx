...
import { Zap, Shield, CheckCircle, TrendingUp, CreditCard, Mail, Star, Users, ArrowRight, Menu, X } from 'lucide-react';
...
import { FormBuilderHero } from './ui/hero-section-8';
...
const LandingPageView = ({ onLoginClick }) => {
  ...
  return (
    <div className="landing-page" data-theme="dark" style={{ 
      minHeight: '100vh', 
      background: '#000000',
      color: '#ffffff',
        ...
        <nav className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 logo">
              <Zap className="logo-icon text-emerald-500" size={28} strokeWidth={3} />
              <span className="text-xl font-bold text-white tracking-tight">CatchUp</span>
            </div>
            ...
            <div className="hidden md:flex items-center justify-center gap-8 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              ...
              <a href="#outcomes" className="text-sm text-white/60 hover:text-white transition-colors">
                Outcomes
              </a>
            ...
            <div className="hidden md:flex items-center gap-4">
              <button 
                ...
                className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md font-medium transition-all text-white/80 hover:text-white h-10 px-4 text-sm cursor-pointer"
              >
                Sign in
              </button>
              <button 
                type="button" 
                onClick={onLoginClick} 
                className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full font-semibold transition-all bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white hover:scale-105 active:scale-95 h-10 px-5 text-sm cursor-pointer shadow-md shadow-emerald-500/20"
              >
                Get started
              </button>
            </div>
            ...
            <div className="px-6 py-4 flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2 pt-4 border-t border-gray-800/50">
                ...
                <button 
                  type="button" 
                  onClick={() => { setMobileMenuOpen(false); onLoginClick(); }} 
                  className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full font-semibold transition-all bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white hover:scale-105 active:scale-95 h-10 px-5 text-sm cursor-pointer shadow-md shadow-emerald-500/10"
                >
                  Get started
                </button>
              </div>
      ...
      <section
        id="product"
        className="relative min-h-screen flex flex-col items-center justify-start px-6 pt-32 pb-20 md:pt-40 md:pb-24"
        style={{
          animation: "fadeIn 0.6s ease-out"
        }}
      >
        {/* Animated Badge */}
        <aside className="mb-8 inline-flex flex-wrap items-center justify-center gap-2 px-4 py-1.5 rounded-full border border-emerald-500/20 bg-emerald-950/20 backdrop-blur-sm max-w-full">
          <span className="text-xs text-center whitespace-nowrap font-medium text-emerald-400/90">
            Never chase unpaid invoices again
          </span>
          <span className="h-3 w-[1px] bg-emerald-500/20" />
          <a
            href="#features"
            className="flex items-center gap-1 text-xs text-emerald-300 hover:text-emerald-100 hover:underline transition-all active:scale-95 whitespace-nowrap"
            aria-label="Read more about our automated recovery"
          >
            Read more
        ...
        {/* CatchUp Headline */}
        <h1
          className="text-4xl md:text-5xl lg:text-7xl font-bold text-center max-w-4xl px-6 leading-tight mb-6 bg-gradient-to-b from-white via-white to-white/60 bg-clip-text text-transparent tracking-[-0.05em]"
        >
          Never let a <span className="bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent font-extrabold">pending deal</span> <br /> slip through the cracks.
        </h1>

        {/* CatchUp Sub-headline */}
        <p className="text-sm md:text-base lg:text-lg text-center max-w-2xl px-6 mb-10 text-gray-400 leading-relaxed">
        ...
        <div className="flex items-center gap-4 relative z-10 mt-6 mb-16">
          <button
            type="button"
            onClick={onLoginClick}
            className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg font-bold transition-all bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white hover:scale-105 active:scale-95 hover:shadow-[0_0_30px_rgba(16,185,129,0.35)] h-12 px-8 text-base cursor-pointer shadow-lg transition-all duration-300"
            aria-label="Start recovering revenue for free"
          >
            Start recovering for free
          </button>
        </div>

        {/* Glowing Background Mesh and App Dashboard mockup */}
        <div className="w-full max-w-5xl relative pb-20">
          ...
          >
            <img
              ...
              style={{ filter: 'hue-rotate(140deg) saturate(1.5) opacity(0.5)' }}
              loading="eager"
            />
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.15)_0%,transparent_70%)] blur-2xl z-0" />
          </div>
          
          <div className="relative z-10">
            <img
              src="https://i.postimg.cc/SKcdVTr1/Dashboard2.png"
              alt="CatchUp Dashboard preview showing recovered metrics and invoices"
      ...
      {/* Trusted By Section with Sparkles and Infinite Slider */}
      <section className="relative w-full overflow-hidden bg-black py-20">
        <div className="mx-auto w-full max-w-2xl px-6">
          <div className="text-center text-3xl font-bold tracking-tight mb-8">
          ...
          <div className="relative h-[100px] w-full z-20">
            <InfiniteSlider className="flex h-full w-full items-center" duration={30} gap={48}>
              {logos.map(({ id, component: Logo, className }) => (
                <div key={id} className={`${className} text-white opacity-70 hover:opacity-100 transition-opacity duration-300`}>
                  <Logo />
                </div>
              ))}
            </InfiniteSlider>
        ...
        {/* Sparkles effect with emerald glow and curve */}
        <div className="relative -mt-20 h-80 w-full overflow-hidden [mask-image:radial-gradient(50%_50%,white,transparent)]">
          <div className="absolute inset-0 before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_bottom_center,#1c9a59,transparent_70%)] before:opacity-30" />
          <div className="absolute -left-1/2 top-1/2 aspect-[1/0.7] z-10 w-[200%] rounded-[100%] border-t border-emerald-500/20 bg-black" />
          <Sparkles
            density={800}
      ...
      <FormBuilderHero
        ...
        description="Join hundreds of freelancers and agencies using CatchUp to recover revenue on autopilot. Free to start, no credit card required."
